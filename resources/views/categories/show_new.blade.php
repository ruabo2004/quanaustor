...
<div class="container">
    ...
    <div class="puma-section-title text-center mb-5">
        ...
        <div style="background: #f8f9fa; padding: 10px; border-radius: 8px; display: inline-block; margin-top: 10px;">
            <small style="color: #495057; font-weight: 600;">
                <i class="fas fa-box"></i> {{ $products->count() }} sản phẩm trong danh mục này
            </small>
        </div>
    </div>
    ...
    <div style="background: #d4edda; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #28a745;">
        <strong style="color: #155724;">✅ Đã lọc thành công:</strong><br>
        <small style="color: #155724;">
            Danh mục: {{ $category->name }} (ID: {{ $category->id }}) | 
            Hiển thị: {{ $products->count() }} sản phẩm thuộc danh mục này
        </small>
    </div>
    @endif
            ...
            <div class="puma-card">
                ...
                <div class="puma-card-body">
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        ...
                        <small style="color: #6c757d; background: #f8f9fa; padding: 2px 6px; border-radius: 4px;">
                    ...
                    <div class="mb-3" style="background: #f8f9fa; padding: 8px; border-radius: 6px;">
                        <small style="color: #6c757d; font-weight: 600;">Tồn kho:</small>
                        <div style="display: flex; gap: 4px; flex-wrap: wrap; margin-top: 4px;">
                            @foreach($product->sizes->take(5) as $size)
                                <span style="
                                    background: {{ $size->stock_quantity <= 0 ? '#dc3545' : ($size->stock_quantity <= 3 ? '#ffc107' : '#28a745') }}; 
                                    color: white; 
                                    padding: 1px 4px; 
                                    border-radius: 3px; 
                                    font-size: 10px; 
                            ...
                            @if($product->sizes && $product->sizes->count() > 0)
                                @foreach($product->sizes->take(5) as $size)
                                    <button type="button" class="quick-size-btn" 
                                            ...
                                            style="width: 24px; height: 24px; border: 1px solid var(--puma-medium-grey); background: white; border-radius: 3px; font-size: 9px; font-weight: 700; cursor: pointer; transition: all 0.2s; {{ !$size->inStock() ? 'opacity: 0.5; cursor: not-allowed;' : '' }}">
                                        {{ $size->size }}
                                    </button>
                                @endforeach
                            @else
    ...
    <div class="text-center py-5">
        ...
        <div style="background: #fff3cd; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #ffc107;">
            <strong style="color: #856404;">Debug Info:</strong><br>
